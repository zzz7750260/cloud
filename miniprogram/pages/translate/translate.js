...
Page({
  ...
  data: {
    ...
    voiceMode:"englishMode",     //声明声音录入模式
    voiceStatus:false,           //声明播放状态
  ...
  */
  getVoice:function(){
    //根据选择模式来选择不同的录入
    let voiceMode = this.data.voiceMode
    let voiceStatus = this.data.voiceStatus

    //根据播放状态voiceStatus来控制录入
    if(voiceStatus === false){
      if (voiceMode === 'englishMode') {
        manager.start({ duration: 30000, lang: "en_US" })
      }
      if (voiceMode === 'chineseMode'){
 ...
 */
  ...
  onLoad: function (options) {
    ...
    manager.onStop = (res) => {
      ...
      if (this.data.voiceMode === "chineseMode"){
        this.chineseToEnglishFunc();
      }
      if (this.data.voiceMode === "englishMode"){
